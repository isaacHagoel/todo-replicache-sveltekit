...
import { withExecutor } from '$lib/server/db/pg.js';
...
describe('data', () => {
	...
	it('getEntry RoundTrip types', async () => {
		await withExecutor(async (executor) => {
	...
	it('delEntry', async () => {
		...
		for (const c of cases) {
			await withExecutor(async (executor) => {
				...
				// eslint-disable-next-line @typescript-eslint/no-explicit-any
				let error: any | undefined;
				await delEntry(executor, 's1', 'foo', 2).catch((e) => (error = String(e)));
